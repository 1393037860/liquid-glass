...
| `data-glass-scatter-neutral` | `<html>` | 让白光散射按内容**饱和度**衰减,值 = 衰减指数(`1` 线性、`2` 更狠),默认 `0`(关)。散射强度 × `(1-饱和度)^指数` → 饱和的选中蓝/彩色照片不被白光冲淡,中性浅色(白/灰)上的玻璃薄雾保持不变。实测指数 `2` 时选中蓝的留白区与真实页面完全同色。 |
